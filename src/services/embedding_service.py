...
from .data_utilizer import DataUtilizer
...
class EmbeddingService:
    def __init__(self, model_name, dataset, dataset_name, 
        ...
        self.data_service = DataUtilizer(dataset_name, index_suffix, text_suffix, caption_suffix)
    ...
    def get_embeddings(self, batch_size=16):
        ...
        index = self.data_service.create_index(img_embeddings)
        self._save_outputs(index, text_embeddings)
        if self.model_name != "CLIP":
            self.data_service.save_captions(self.model.get_captions())
    ...
    def get_embeddings_with_checkpoint(self, batch_size=16, checkpoint_size=64, check_database=False):
        ...
        caption_embedding = []
        all_text_embedding = []
        if check_database and self.data_service.is_img_embd_exists(self.data_service._get_img_index_path):
            index = self.data_service.load_imgs_index()
            start_index = max(index.ntotal, 0)
        if check_database and self.data_service.is_text_embd_exists(self.data_service._get_text_embd_path):
            all_text_embedding = [self.data_service.load_texts_embeddings()]
        while start_index < dataset_len - 1:
            ...
            if self.model_name != "CLIP":
                ...
                self.data_service.save_captions(caption_embedding)
    ...
    def _save_outputs(self, index, text_embeddings):
        self.data_service.save_index(index)
        self.data_service.save_texts_embeddings(text_embeddings)
