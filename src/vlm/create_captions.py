...
# Set up
model_cache_dir = settings.models_cache_dir
output_file = os.path.join(settings.output_dir, "response_dict_long_1_keyword.json")
...
prompt = """[INST] <image>\nWrite a detailed caption (2-3 sentences) for the photo. Consider the output template when you respond. Do not generate anything else. Here is the output template:
    {"caption": "detailed description of the photo"} Take a deep breath and answer only with a JSON. [/INST]"""
    
prompt = """[INST] <image>\nWrite a series of unique keywords for the photo as caption. Keywords should contain objects and actions. Consider the output template when you respond. Do not generate anything else. Here is the output template:
...
for d in file_restriction["images"][:500]:
    ...
    image_path = os.path.join(settings.data_dir, "coco/images/val2017", filename)
